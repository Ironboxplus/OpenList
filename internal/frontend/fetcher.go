...
func getFrontendRepo() string {
	if conf.Conf != nil && strings.TrimSpace(conf.Conf.FrontendRepo) != "" {
		return strings.TrimSpace(conf.Conf.FrontendRepo)
	}
	return defaultFrontendRepo
}
...
func resolveTagCommitSHA(ctx context.Context, client *http.Client, baseURL, tag string) (string, error) {
	...
	repo := getFrontendRepo()
	refURL := fmt.Sprintf("%s/repos/%s/git/ref/tags/%s", apiBase, repo, url.PathEscape(tag))
	...
	case "tag":
		...
		tagObjURL := fmt.Sprintf("%s/repos/%s/git/tags/%s", apiBase, repo, ref.Object.SHA)
...
func fetchFromTag(ctx context.Context, tag string, baseURL string) (*FetchResult, error) {
	...
	if baseURL != "" {
		if tag == "" {
			...
			apiURL = fmt.Sprintf("%s/repos/%s/releases/tags/%s", baseURL, repo, tag)
		}
	} else {
		if tag == "" {
