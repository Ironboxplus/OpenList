...
func (r *RefreshableRangeReader) RangeRead(ctx context.Context, httpRange http_range.Range) (io.ReadCloser, error) {
	...
	// Wrap the ReadCloser to handle errors during reading
	return &refreshableReadCloser{
		ReadCloser:   rc,
		ctx:          ctx,
		parent:       r,
		currentRange: httpRange,
		bytesRead:    0,
	}, nil
}
...
// refreshableReadCloser wraps an io.ReadCloser to handle mid-stream errors
type refreshableReadCloser struct {
	io.ReadCloser
	ctx          context.Context
	parent       *RefreshableRangeReader
	currentRange http_range.Range
	bytesRead    int64
	closed       bool
}

func (r *refreshableReadCloser) Read(p []byte) (n int, err error) {
	if r.closed {
	...
	if err != nil && err != io.EOF {
		// Check if this is a link expired error during reading
		if IsLinkExpiredError(err) && r.parent.link.Refresher != nil {
			log.Warnf("Link expired during read at offset %d, attempting to refresh...", r.currentRange.Start+r.bytesRead)

			// Close the old reader
			r.ReadCloser.Close()

			// Calculate new range from current position
			newRange := http_range.Range{
				Start:  r.currentRange.Start + r.bytesRead,
				Length: r.currentRange.Length - r.bytesRead,
			}
			if r.currentRange.Length < 0 {
				newRange.Length = -1
			}

			// Try to refresh and get new reader
			newRC, refreshErr := r.parent.refreshAndRetry(r.ctx, newRange)
			if refreshErr != nil {
				return n, fmt.Errorf("failed to refresh link during read: %w (original error: %v)", refreshErr, err)
			}

			// Replace the reader and reset bytesRead for the new range
			r.ReadCloser = newRC
			r.currentRange = newRange
			r.bytesRead = 0

			log.Infof("Link refreshed, continuing read from offset %d", newRange.Start)

			// If we already read some data, return it; next Read will use new reader
			if n > 0 {
				return n, nil
			}

			// Otherwise, try reading from the new reader immediately
...
func (r *refreshableReadCloser) Close() error {
	if r.closed {
		return nil
	}
	r.closed = true
